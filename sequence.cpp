...
bool sequence::init (const seqchar *buffer, size_t length)
{
	...
	span *sptr = new span(0, length, /*bc->id*/ NULL, tail, head); //!
	head->next = sptr;
	tail->prev = sptr;
...
bool sequence::erase_worker (size_w index, size_w length, action act)
{
	...
	if(remoffset != 0)
	{
		...
		newspans.append(new span(sptr->offset, remoffset, /*sptr->buffer*/ sptr->pDS));
...
size_w sequence::render(size_w index, seqchar *dest, size_w length) const
{
	...
	while(length && sptr != tail)
	{
        ...
        sptr->pDS->Read(sptr->offset + spanoffset, copylen * sizeof(seqchar), (uint8*)dest);
...
bool sequence::CanReplaceAt(size_w nIndex, size_w nOldSize, size_w nNewSize)
{
    return false;
}

bool sequence::InsertSegment(size_w nAddress, size_w nSize, size_w stored_address, DataSource *pDS)
{
    // only works once.  DON'T CALL THIS.
    char c = 0x5a;
    init((const seqchar*)&c, 1);
    span *sptr = head->next;
    sptr->length = nSize;
    sptr->offset = nAddress;
    sptr->pDS = pDS;
    this->sequence_length = nSize;
    return true;
}

bool sequence::IsModified() const
{
    return false;
}

bool sequence::init(size_w nAddress, size_w length, size_w stored_address, DataSource *pDS)
{
	clear();
	...
	//memcpy(bc->buffer, buffer, length * sizeof(seqchar));
	//bc->length = length;
    //scratch = new DSModifyBuffer();

    span *sptr = new span(0, length, pDS, tail, head);
	head->next = sptr;
	tail->prev = sptr;
