...
#ifndef AKRIP_EXPORT
#ifdef AKRIP_DLL_EXPORTS
#define AKRIP_EXPORT __declspec(dllexport)
#else
#define AKRIP_EXPORT __declspec(dllimport)
#endif // AKRIP_DLL_EXPORTS
#endif // AKRIP_EXPORT

int AKRIP_EXPORT GetNumAdapters( void );
int AKRIP_EXPORT GetCDList( LPCDLIST cd );
int AKRIP_EXPORT GetAspiLibError( void );
BYTE AKRIP_EXPORT GetAspiLibAspiError( void );

DWORD AKRIP_EXPORT GetCDId( HCDROM hCD, char *buf, int maxBuf );
DWORD AKRIP_EXPORT GetDriveInfo( BYTE ha, BYTE tgt, BYTE lun, LPCDREC cdrec );
DWORD AKRIP_EXPORT ReadTOC( HCDROM hCD, LPTOC lpToc );
DWORD AKRIP_EXPORT ReadCDAudioLBA( HCDROM hCD, LPTRACKBUF );
BOOL AKRIP_EXPORT QueryCDParms( HCDROM hCD, int which, DWORD *pNum );
BOOL AKRIP_EXPORT ModifyCDParms( HCDROM hCD, int which, DWORD val );
HCDROM AKRIP_EXPORT GetCDHandle( LPGETCDHAND lpcd );
BOOL AKRIP_EXPORT CloseCDHandle( HCDROM );
DWORD AKRIP_EXPORT ReadCDAudioLBAEx( HCDROM hCD, LPTRACKBUF, LPTRACKBUF );
DWORD AKRIP_EXPORT GetAKRipDllVersion( void );
DWORD AKRIP_EXPORT GetCDCapacity( BYTE ha, BYTE tgt, BYTE lun, PLARGE_INTEGER pCapacity );
