...
int InitSCSIPT( void )
{
  ...
  for( i = 2; i < 26; i++ )
	...
	{
	  SPT_GetDriveInformation( i, &sptiglobal.drive[i] );
...
#ifdef _DEBUG_SCSIPT
  if ( fh == INVALID_HANDLE_VALUE )
    dbprintf( "akrip32: scsipt: CreateFile() failed! -> %dn", GetLastError() );
  else
    dbprintf( "akrip32: scsipt: CreateFile() returned %d\n", GetLastError() );
#endif
...
{
#ifdef _DEBUG_SCSIPT
  dbprintf( "AKRip32: SPTIGetDeviceType( %d:%d:%d \n)",lpsrb->SRB_HaID, lpsrb->SRB_Target, lpsrb->SRB_Lun );
#endif
