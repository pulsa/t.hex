...
#ifdef _DBLIBMAIN
      dbprintf( "akrip32: DLL_PROCESS_ATTACH (hModule=%04X)\n", hModule );
#endif
      loadAspi();
...
#ifdef _DBLIBMAIN
      dbprintf( "akrip32: DLL_THREAD_ATTACH (hModule=%04X)\n", hModule );
#endif
      retVal = TRUE;
      break;
...
#ifdef _DBLIBMAIN
      dbprintf( "akrip32: DLL_THREAD_DETACH (hModule=%04X)\n", hModule );
#endif
      retVal = TRUE;
      break;
  ...
  switch( dwStatus )
    ...
    case SS_COMP:
      ...
      dbprintf( "akrip32: getSCSIDevType() -> SS_COMP: %04X\n",
		s.SRB_DeviceType );
      dbprintf( "akrip32: getSCSIDevType(): pDevType == 0x%08X\n", pDevType );
#endif
      if ( pDevType )
	...
	*pDevType = 0x1F;
      if ( lpDevTypeStr )
	strncpy( lpDevTypeStr, "SS_NO_DEVICE\n", iDevTypeLen );
      return FALSE;

    default:
#ifdef _DEBUG
      dbprintf( "akrip32: getSCSIDevType(): unexpected return from ASPI (%04X)\n", dwStatus );
#endif
      if ( pDevType )
	*pDevType = 0x1F;
      if ( lpDevTypeStr )
  ...
  if ( hinstWNASPI32 )
      ...
      if ( !pfnGetASPI32SupportInfo || !pfnSendASPI32Command )
	{
	  ...
	  dbprintf( "akrip32: GetASPI32SupportInfo == 0x%08X\n", 
		   pfnGetASPI32SupportInfo );
	  dbprintf( "akrip32: SendASPI32Command == 0x%08X\n", pfnSendASPI32Command );
#endif
	  return 0;
	}
      dwAPI = APIC_ASPI;
  ...
  else
    {
      ...
      dbprintf( "akrip32: GetLastError() -> %d (%04X)\n", dwErr, dwErr );
#endif
      dwAPI = APIC_NONE;
...
#ifdef _DEBUG
      dbprintf( "akrip32: GetNumAdapters: bASPIStatus == 0x%02X\n", bASPIStatus );
#endif
      return -1;
    }
  return (int)bHACount;
...
DWORD GetDriveInfo( BYTE ha, BYTE tgt, BYTE lun, LPCDREC cdrec )
{
  ...
  wsprintf( outBuf, "%s, %s v%s (%d:%d:%d)", 
	    ...
	    ha, tgt, lun );
#ifdef _DEBUG
  dbprintf( "akrip32: GetDriveInfo: %s\n", outBuf );
#endif
...
#ifdef _DEBUG
  dbprintf( "AKRip32: GetCDList(): numAdapters == %d\n", numAdapters );
#endif
  if ( numAdapters == 0 )
    {
...
DWORD readCDAudioLBA_ANY( HCDROM hCD, LPTRACKBUF t )
{
  ...
  for( i = 0; i < 7; i++ )
    {
#ifdef _DEBUG
      dbprintf( "akrip32: testing read fn %d\n", ord[i] );
#endif
...
#ifdef _DEBUG
	  dbprintf( "rolling back start frame to %d\n", t->startFrame );
#endif
	}
      else
	{
...
#ifdef _DEBUG
  dbprintf( "akrip32: readCDAudioLBAEx: returning %04X\n", retVal );
#endif
 ...
 * GetDriveInfo
 ***************************************************************************/
DWORD GetCDCapacity( BYTE ha, BYTE tgt, BYTE lun, PLARGE_INTEGER pCapacity )
{
  DWORD dwStatus;
  HANDLE heventSRB;
  SRB_ExecSCSICmd s;
...
#ifdef _DEBUG
      dbprintf( "akrip32: GetCDCapacity: Error status: 0x%04X\n\n", s.SRB_Status );
#endif
      return SS_ERR;
    }

  capacity = (__int64)(MSB2DWORD2(buf) + 1) * MSB2DWORD2(buf+4);
#ifdef _DEBUG
  dbprintf( "akrip32: GetCDCapacity: %I64d\n", capacity );
#endif

  if ( pCapacity )
    pCapacity->QuadPart = capacity;
