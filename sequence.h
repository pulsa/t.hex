...
public:
    ...
    bool Read(size_w nOffset, size_t nSize, uint8 *target, uint8 *pModified = NULL)
    {
        if (pModified)
            memset(pModified, 0, nSize);
        return render(nOffset, (seqchar*)target, nSize) == nSize;
    }

    bool IsModified() const;
    size_w GetSize() const { return size(); }
    bool CanReplaceAt(size_w nIndex, size_w nOldSize, size_w nNewSize);
    //bool InsertSegment(THSIZE nAddress, Segment *ts); //! always returns true for now
    bool InsertSegment(size_w nAddress, size_w nSize, size_w stored_address, DataSource *pDS);
    ...
    bool init(size_w offset, size_w length, size_w display_address, DataSource *pDS);
    DataSource *scratch;
};
...
public:
	// constructor
	span(size_w off, size_w len, /*int buf*/ DataSource *ds, span *nx = 0, span *pr = 0) 
			:
			next(nx), 
			prev(pr),
			offset(off), 
