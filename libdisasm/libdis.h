...
#define LIBDISASM_API  // don't use DLL interface

#ifndef LIBDISASM_API
    #ifdef LIBDISASM_EXPORTS
...
LIBDISASM_API int x86_init( enum x86_options options, DISASM_REPORTER reporter, void *arg);
LIBDISASM_API void x86_set_reporter( DISASM_REPORTER reporter, void *arg);
LIBDISASM_API void x86_set_options( enum x86_options options );
LIBDISASM_API enum x86_options x86_get_options( void );
LIBDISASM_API int x86_cleanup(void);
...
LIBDISASM_API int x86_insn_is_valid( x86_insn_t *insn );
...
/* Mark an instruction's operand list as unused.  This avoids unnecessary memory allocations.
 * Call x86_oplist_free() when you're really done with an x86_insn_t. (AEB) */
LIBDISASM_API
void x86_oplist_empty( x86_insn_t *insn );
